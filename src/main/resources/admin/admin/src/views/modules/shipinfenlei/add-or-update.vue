<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px 0","background":"url(http://codegen.caihongy.cn/20230328/e3b3ef59d7a44ae6afcff886f7a76b98.jpg) no-repeat center bottom / 100% 100%","height":"auto"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="视频分类" prop="shipinfenlei">
					<el-input v-model="ruleForm.shipinfenlei" placeholder="视频分类" clearable  :readonly="ro.shipinfenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="视频分类" prop="shipinfenlei">
					<el-input v-model="ruleForm.shipinfenlei" placeholder="视频分类" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shipinfenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shipinfenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
